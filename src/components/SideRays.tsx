/**
 * Side Rays — React Bits (https://reactbits.dev)
 */
...
export interface SideRaysProps {
  ...
  /** 0 = source à gauche, 1 = à droite (trajectoire du soleil). */
  sourceX?: number;
  flipX?: number;
  flipY?: number;
  tilt?: number;
...
const SideRays = ({
  ...
  const resolvedSourceX = sourceX ?? (flipX !== undefined ? 1 - flipX : 1 - originToFlip(origin)[0]);
  const resolvedFlipX = flipX ?? originToFlip(origin)[0];
  const resolvedFlipY = flipY ?? originToFlip(origin)[1];
...
void main() {
  ...
  float sx = clamp(iSourceX, 0.0, 1.0);
  vec2 rayPos = vec2(iResolution.x * mix(-0.14, 1.14, sx), -0.48 * iResolution.y);
  ...
  gl_FragColor = color;
      ...
      const uniforms: Record<string, { value: number | number[] }> = {
        ...
        iSourceX: { value: resolvedSourceX },
        iTilt: { value: tilt },
  ...
  useEffect(() => {
    ...
    u.iSourceX.value = resolvedSourceX;
    u.iTilt.value = tilt;
